...
-(void)getCheckData{
    ...
    [dataService getPrCheckPlanDataWithAccessToken:AccessToken operationTime:[PowerUtils getOperationTime:@"yyyy-MM-dd-HH:mm:ss"] checkStatusNum:@"4" completeBlock:block loadingView:checkDataTableView showHUD:YES];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    //delegate响应弹出二级任务列表
    if(_delegate && [_delegate respondsToSelector:@selector(popYiwanchengDetailViewController:titleLabel:)]) {
        NSDictionary *checkDataDic = [checkDataList objectAtIndex:indexPath.row];
        [_delegate popYiwanchengDetailViewController:[checkDataDic objectForKey:@"PLAN_NUM"] titleLabel:[checkDataDic objectForKey:@"PLAN_NAME"]];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    endStartLabel.text = [NSString stringWithFormat:@"完成时间:%@",[dic objectForKey:@"STOP_TIME"]];
    //设置数量
