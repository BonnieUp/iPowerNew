...
@interface GroupTaskViewController ()

@end

@implementation GroupTaskViewController{
    NSString *isAuto;
}
...
-(void)getTask {
    ...
    RequestCompleteBlock block = ^(id result) {
        ...
        if(_delegate && [_delegate respondsToSelector:@selector(setGroupTaskAmount:)]) {
            [_delegate setGroupTaskAmount:[NSString stringWithFormat:@"%d",taskList.count ]];
        }
    ...
    [service getTaskAmountBySiteWithAccessToken:AccessToken operationTime:[PowerUtils getOperationTime:@"yyyy-MM-dd-HH:mm:ss"] taskStat:@"1" siteId:_siteId resNo:nil taskKind:nil taskScope:@"2" startDate:[PowerUtils getFirstDay:@"yyyy-MM-dd"] endDate:[PowerUtils getLastDay:@"yyyy-MM-dd"] completeBlock:block loadingView:taskTableView showHUD:YES];
}
...
-(void)getTaskWithResNo:(NSString *)resNo {
    ...
    PowerDataService *service = [[PowerDataService alloc]init];
    [service getTaskAmountBySiteWithAccessToken:AccessToken operationTime:[PowerUtils getOperationTime:@"yyyy-MM-dd-HH:mm:ss"] taskStat:@"1" siteId:_siteId resNo:resNo taskKind:nil taskScope:@"2" startDate:[PowerUtils getFirstDay:@"yyyy-MM-dd"] endDate:[PowerUtils getLastDay:@"yyyy-MM-dd"] completeBlock:block loadingView:taskTableView showHUD:YES];
}
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(_delegate && [_delegate respondsToSelector:@selector(popSubTaskListWith:taskTypeId:taskName:taskAmount: isAuto:)]) {
        ...
        [_delegate popGroupSubTaskListWith:_siteId taskTypeId:[taskDic objectForKey:@"taskTypeId"] taskName:[taskDic objectForKey:@"taskTypeName"] taskAmount:[taskDic objectForKey:@"taskAmount"] isAuto:isAuto];
    }
}
