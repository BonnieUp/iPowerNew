//
//  XudianchineizuceshiViewController.m
//  i动力
//
...
@implementation XudianchineizuceshiViewController
...
-(void)popSearchWindow{
    if(_delegate && [_delegate respondsToSelector:@selector(popNeizuFilterConditionController)]){
        [_delegate popNeizuFilterConditionController];
    }
}
...
-(void)setRegNO:(NSString *)regNO{
    if(![_regNO isEqualToString:regNO]) {
        _regNO = regNO;
        [self getXudianchiDataWithRegNum:_regNO testTypeNum:@"2" staName:@"" sysName:@"" assetsNum:@"" articleNum:@"" startTime:[PowerUtils getFirstDay:@"yyyy-MM-dd-HH:mm:ss"] stopTime:[PowerUtils getOperationTime:@"yyyy-MM-dd-HH:mm:ss"]];
    }
}
...
- (CGFloat)tableView:(UITableView *)tView heightForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(detailDic!=nil){
        ...
        if(commonInfoArray.count%2==0){
            height+= 20*commonInfoArray.count/2;
        }else{
            height+= 20*(commonInfoArray.count/2+1);
        }
        if(extendInfoArray.count%2==0){
            height+= 20*extendInfoArray.count/2;
        }else{
            height+= 20*(extendInfoArray.count/2+1);
        }
        height+=40;
    }
    return height;
...
-(void)openDetailView:(id)sender{
    ...
    if([@"0" isEqualToString:[dic objectForKey:@"open"]]){
        ...
        if(detailDic==nil){
            ...
            PowerDataService *dataService = [[PowerDataService alloc]initUserJsonHead];
            [dataService getPrBatteryInnerResistanceTestDetailWithAccessToken:AccessToken operationTime:[PowerUtils getOperationTime:@"yyyy-MM-dd-HH:mm:ss"] objNum:[dic objectForKey:@"OBJ_NUM"] completeBlock:block loadingView:tableView showHUD:YES];
...
-(UITableViewCell *)tableView:(UITableView *)tView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    NSString *iden = @"cellneizu";
    UITableViewCell *cell = [tView dequeueReusableCellWithIdentifier:iden];
    cell = [[UITableViewCell alloc]initWithStyle:UITableViewCellStyleDefault reuseIdentifier:iden];
    NSMutableDictionary *dic = [batteryList objectAtIndex:indexPath.section];
    NSMutableDictionary *detailDic = [dic objectForKey:@"detailDic"];
    if(detailDic!=nil){
        UILabel *gongongLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth, 20)];
        gongongLabel.text = @"公共属性";
        gongongLabel.font = [UIFont boldSystemFontOfSize:12];
        [cell.contentView addSubview:gongongLabel];
        NSArray *commonInfoArray = [detailDic objectForKey:@"CommonInfoList"];
        NSArray *extendInfoArray = [detailDic objectForKey:@"ExtenderInfoList"];
        int yPosition = 20;
        ...
        UILabel *kuozhanLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, yPosition+20, ScreenWidth, 20)];
        kuozhanLabel.font = [UIFont boldSystemFontOfSize:12];
        kuozhanLabel.text = @"扩展属性";
        [cell.contentView addSubview:kuozhanLabel];
        yPosition+=40;
        for(int i=0;i<extendInfoArray.count;i++){
            NSDictionary *detailItemDic = [extendInfoArray objectAtIndex:i];
            CGRect labelFrame;
            if(i%2==0 && i!=0){
                yPosition+=20;
            }
            //左边
            if(i%2==0){
                labelFrame = CGRectMake(0, yPosition, ScreenWidth/2, 20);
            }else{
                labelFrame = CGRectMake(ScreenWidth/2, yPosition, ScreenWidth/2, 20);
            }
            UILabel *label = [[UILabel alloc]initWithFrame:labelFrame];
            [label setTextAlignment:NSTextAlignmentLeft];
            [label setFont:[UIFont systemFontOfSize:11]];
