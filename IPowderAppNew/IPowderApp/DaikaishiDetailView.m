...
@implementation DaikaishiDetailView{
...
-(void)startHandler:(id)sender{
    ...
    [dataService excutePrCheckStationWithAccessToken:AccessToken operationTime:[PowerUtils getOperationTime:@"yyyy-MM-dd-HH:mm:ss"] operStatusNum:@"2" recordNumList:[self getSelectRecordNum] completeBlock:block loadingView:scrollView showHUD:YES];
}
...
-(void)setPlanNum:(NSString *)num{
        ...
        PowerDataService *dataService = [[PowerDataService alloc]initUserJsonHead];
        [dataService getPrCheckStationDataWithAccessToken:AccessToken operationTime:[PowerUtils getOperationTime:@"yyyy-MM-dd-HH:mm:ss"] checkStatusNum:@"2" planNum:planNum completeBlock:block loadingView:self.view showHUD:YES];
}
...
-(UIView *)createSectionView:(NSArray *)sectionArray{
    ...
    //headView
    UIView *headView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, ScreenWidth-4, 30)];
    UILabel *leftView = [[UILabel alloc]initWithFrame:CGRectMake(10, 0, (ScreenWidth-120), 30)];
    leftView.textAlignment = NSTextAlignmentLeft;
    leftView.font = [UIFont systemFontOfSize:14];
    [headView addSubview:leftView];
    
    UILabel *rightView = [[UILabel alloc]initWithFrame:CGRectMake(ScreenWidth-120, 0, 110, 30)];
    ...
    //contentView
    UIView *contentView = [[UIView alloc]initWithFrame:CGRectMake(0, 30, ScreenWidth-4, rowHeight*sectionArray.count)];
    float yPosition = 0;
    //sectionArray按照首字母排序
    NSComparator cmptr = ^(id obj1, id obj2){
    ...
    for(NSMutableDictionary *pieceDic in sortSectionArray){
        ...
        selectBtn.frame = CGRectMake(5,5,ScreenWidth-5,60 );
        //checkbox状态
        ...
        //局站label
        UILabel *juzhanLabel = [[UILabel alloc]initWithFrame:CGRectMake(35, 5, ScreenWidth-35, 20)];
        juzhanLabel.font =[UIFont boldSystemFontOfSize:13];
        juzhanLabel.text = [pieceDic objectForKey:@"STATION_NAME"];
        [pieceView addSubview:juzhanLabel];
        
        //联系人label
        UILabel *lianxirenLabel = [[UILabel alloc]initWithFrame:CGRectMake(35, 25, ScreenWidth-35, 20)];
        lianxirenLabel.font = [UIFont systemFontOfSize:12];
        lianxirenLabel.text = [NSString stringWithFormat:@"联系人:%@(%@)",[pieceDic objectForKey:@"CONTACT_PERSON"],[pieceDic objectForKey:@"CONTACT_TEL"]];
        [pieceView addSubview:lianxirenLabel];
        
        //地址label
        UILabel *dizhiLabel = [[UILabel alloc]initWithFrame:CGRectMake(35, 40, ScreenWidth-35, 20)];
        dizhiLabel.font = [UIFont systemFontOfSize:12];
        dizhiLabel.text = [NSString stringWithFormat:@"局站地址:%@",[pieceDic objectForKey:@"STATION_ADDR"]];
        [pieceView addSubview:dizhiLabel];
...
-(CGFloat)caculateSectionHeight:(NSArray *)sectionArray{
    return sectionArray.count *rowHeight + 30;
}
