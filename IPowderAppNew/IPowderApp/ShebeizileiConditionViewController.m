//
//  ShebeizileiConditionViewController.m
//  IPowerApp
//
...
@interface ShebeizileiConditionViewController ()

@end

@implementation ShebeizileiConditionViewController {
...
-(void)createZileiBtn {
    float cellHei = 35;
    float maxHei = cellHei * _zileiList.count + 90;
    ...
    if(maxHei < aviMaxHei ) {
        ...
        btnContainerView.contentSize = CGSizeMake(260, _zileiList.count*cellHei+10);
        submitBtn.frame = CGRectMake(10,btnContainerView.bottom+5,containerView.width-20,30);
    }else {
        containerView.frame = CGRectMake((ScreenWidth-280)/2, 5, 280, aviMaxHei);
        btnContainerView.frame = CGRectMake(10, 40, 260, aviMaxHei-90);
        btnContainerView.contentSize = CGSizeMake(260, _zileiList.count*cellHei);
        submitBtn.frame = CGRectMake(10,btnContainerView.bottom+5,containerView.width-20,30);
    }
    
    //生成设备大类子按钮
    for(int i=0;i<_zileiList.count;i++) {
        NSDictionary *cellDic = [_zileiList objectAtIndex:i];
        ...
        [cellBtn setTitle:[cellDic objectForKey:@"DEV_SUBCLASS_NAME"] forState:UIControlStateNormal];
        ...
        [cellBtn addTarget:self action:@selector(shebeizileiBtnOnClick:) forControlEvents:UIControlEventTouchUpInside];
        cellBtn.backgroundColor = [UIColor clearColor];
        [btnContainerView addSubview:cellBtn];
        
        [cells addObject:cellBtn];
        //添加分割线
        if(i!= _zileiList.count -1 ) {
...
//设备大类里的字按钮点击事件
-(void)shebeizileiBtnOnClick:(UIButton *)btn {
    [self resetAllBtn];
    //将选中设置为选中状态
...
-(void)submitHandler {
    if(selectIndex>=0) {
        NSDictionary *selectDic = [_zileiList objectAtIndex:selectIndex];
        if(_delegate && [_delegate respondsToSelector:@selector(submitShebeizileiCondition:)]) {
            [_delegate submitShebeizileiCondition:selectDic];
            //关闭窗口
            [self disappearWinow];
        }
    }
...
-(void)setZileiList:(NSArray *)zileiList {
    if(![_zileiList isEqual:zileiList ]) {
        _zileiList = zileiList;
...
-(void)setSelectZilei:(NSString *)selectZilei {
    if(![_selectZilei isEqualToString:selectZilei ]) {
        _selectZilei = selectZilei;
        NSInteger index;
        for (NSDictionary* dic in _zileiList) {
            if([[dic objectForKey:@"DEV_SUBCLASS_NUM"] isEqualToString:selectZilei] ) {
                index = [_zileiList indexOfObject:dic];
                break;
            }
        }
