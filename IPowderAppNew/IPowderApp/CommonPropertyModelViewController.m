//
//  CommonPropertyModelViewController.m
//  IPowerApp
//
...
@interface CommonPropertyModelViewController ()

@end

@implementation CommonPropertyModelViewController {
...
- (void)viewDidLoad
{
    ...
    //名称
    titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(5, 10, 250, 30)];
...
-(void)createZileiBtn {
    float cellHei = 35;
    float maxHei = cellHei * _dataList.count + 90;
    ...
    if(maxHei < aviMaxHei ) {
        ...
        btnContainerView.contentSize = CGSizeMake(260, _dataList.count*cellHei+10);
        submitBtn.frame = CGRectMake(10,btnContainerView.bottom+5,containerView.width-20,30);
    }else {
        containerView.frame = CGRectMake((ScreenWidth-280)/2, 5, 280, aviMaxHei);
        btnContainerView.frame = CGRectMake(10, 40, 260, aviMaxHei-90);
        btnContainerView.contentSize = CGSizeMake(260, _dataList.count*cellHei);
        submitBtn.frame = CGRectMake(10,btnContainerView.bottom+5,containerView.width-20,30);
    }
    
    //生成设备大类子按钮
    for(int i=0;i<_dataList.count;i++) {
        NSDictionary *cellDic = [_dataList objectAtIndex:i];
        UIButton *cellBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        cellBtn.frame = CGRectMake(5, i*cellHei, 250, cellHei);
        ...
        [cellBtn addTarget:self action:@selector(cellBtnHandler:) forControlEvents:UIControlEventTouchUpInside];
...
-(void)setSelectDic:(NSMutableDictionary *)selectDic{
    if(![_selectDic isEqual:selectDic ]) {
        _selectDic = selectDic;
        NSInteger index=-1;
        for (NSDictionary* dic in _dataList) {
            if([[dic objectForKey:@"value"] isEqualToString:[selectDic objectForKey:@"value"]] ) {
                index = [_dataList indexOfObject:dic];
                break;
            }
        }
...
//设备大类里的字按钮点击事件
-(void)cellBtnHandler:(UIButton *)btn {
    [self resetAllBtn];
    //将选中设置为选中状态
    [self setSelectState:btn];
...
-(void)submitHandler {
    if(selectIndex>=0) {
        NSDictionary *selectDic = [_dataList objectAtIndex:selectIndex];
        if(_delegate && [_delegate respondsToSelector:@selector(submitHandler:)]) {
            [_delegate submitHandler:selectDic];
            //关闭窗口
            [self disappearWinow];
        }
    }
