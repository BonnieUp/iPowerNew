//
//  SuoshufangjianConditionViewController.m
//  IPowerApp
//
...
@interface SuoshufangjianConditionViewController () {
...
@implementation SuoshufangjianConditionViewController
...
//创建子类按钮
-(void)createSuoshufangjianBtn {
    float cellHei = 35;
    float maxHei = cellHei * _suoshufangjianList.count + 90;
    ...
    if(maxHei < aviMaxHei ) {
        ...
        btnContainerView.contentSize = CGSizeMake(260, _suoshufangjianList.count*cellHei+10);
        submitBtn.frame = CGRectMake(10,btnContainerView.bottom+5,containerView.width-20,30);
    }else {
        containerView.frame = CGRectMake((ScreenWidth-280)/2, 5, 280, aviMaxHei);
        btnContainerView.frame = CGRectMake(10, 40, 260, aviMaxHei-90);
        btnContainerView.contentSize = CGSizeMake(260, _suoshufangjianList.count*cellHei);
        submitBtn.frame = CGRectMake(10,btnContainerView.bottom+5,containerView.width-20,30);
    }
    
    //生成设备大类子按钮
    for(int i=0;i<_suoshufangjianList.count;i++) {
        NSDictionary *cellDic = [_suoshufangjianList objectAtIndex:i];
        ...
        [cellBtn setTitle:[cellDic objectForKey:@"ROOM_NAME"] forState:UIControlStateNormal];
        ...
        [cellBtn addTarget:self action:@selector(suoshufangjianBtnOnClick:) forControlEvents:UIControlEventTouchUpInside];
        cellBtn.backgroundColor = [UIColor clearColor];
        [btnContainerView addSubview:cellBtn];
        
        [cells addObject:cellBtn];
        //添加分割线
        if(i!= _suoshufangjianList.count -1 ) {
...
//设备大类里的字按钮点击事件
-(void)suoshufangjianBtnOnClick:(UIButton *)btn {
    [self resetAllBtn];
    //将选中设置为选中状态
...
-(void)submitHandler {
    if(selectIndex>=0) {
        NSDictionary *selectDic = [_suoshufangjianList objectAtIndex:selectIndex];
        if(_delegate && [_delegate respondsToSelector:@selector(submitSuoshufangjianCondition:)]) {
            [_delegate submitSuoshufangjianCondition:selectDic];
            //关闭窗口
            [self disappearWinow];
        }
    }
...
-(void)setSuoshufangjianList:(NSArray *)suoshufangjianList{
    if(![_suoshufangjianList isEqual:suoshufangjianList ]) {
        _suoshufangjianList = suoshufangjianList;
        
        //根据数据源生成按钮
        [self createSuoshufangjianBtn];
    }
}

-(void)setSelectSuoshufangjian:(NSString *)selectSuoshufangjian{
    if(![_selectSuoshufangjian isEqualToString:selectSuoshufangjian ]) {
        _selectSuoshufangjian = selectSuoshufangjian;
        NSInteger index;
        for (NSDictionary* dic in _suoshufangjianList) {
            if([[dic objectForKey:@"ROOM_NUM"] isEqualToString:selectSuoshufangjian] ) {
                index = [_suoshufangjianList indexOfObject:dic];
                break;
            }
        }
