...
@implementation DaiwanchengViewController
...
-(void)getCheckData{
    ...
    [dataService getPrCheckPlanDataWithAccessToken:AccessToken operationTime:[PowerUtils getOperationTime:@"yyyy-MM-dd-HH:mm:ss"] checkStatusNum:@"3" completeBlock:block loadingView:checkDataTableView showHUD:YES];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    //delegate响应弹出二级任务列表
    if(_delegate && [_delegate respondsToSelector:@selector(popDaiwanchengDetailViewController:titleLabel:)]) {
        NSDictionary *checkDataDic = [checkDataList objectAtIndex:indexPath.row];
        [_delegate popDaiwanchengDetailViewController:[checkDataDic objectForKey:@"PLAN_NUM"] titleLabel:[checkDataDic objectForKey:@"PLAN_NAME"]];
    }
...
//        [cell.contentView addSubview:endTimeLabel];
    ...
    timeStartLabel.text = [NSString stringWithFormat:@"开始时间:%@",[dic objectForKey:@"START_TIME"]];
    //设置结束时间
//    UILabel *endStartLabel = (UILabel *)[cell.contentView viewWithTag:102];
//    endStartLabel.text = [NSString stringWithFormat:@"计划开始日期:%@",[dic objectForKey:@"STOP_TIME"]];
    //设置数量
