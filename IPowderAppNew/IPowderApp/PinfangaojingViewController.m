...
@interface PinfangaojingViewController ()

@end

@implementation PinfangaojingViewController {
...
- (void)viewDidLoad
{
    ...
    [self.view addSubview:gaojingTable];
     ...
     [self getPinfangaojingWithregNum:@"" stationName:@"" devName:@"" pointName:@"" alarmCount:@"" startTime:[PowerUtils getZeroDay:@"yyyy-MM-dd-HH:mm:ss"] stopTime:[PowerUtils getOperationTime:@"yyyy-MM-dd-HH:mm:ss"]];
}
...
-(void)getPinfangaojingWithregNum:(NSString *)regNum
    ...
    RequestCompleteBlock block = ^(id result ) {
        ...
        if(_delegate && [_delegate respondsToSelector:@selector(setPinfangaojingAmount:)]) {
            
            [_delegate setPinfangaojingAmount:[NSString stringWithFormat:@"%i",gaojingList.count]];
        }
    };
