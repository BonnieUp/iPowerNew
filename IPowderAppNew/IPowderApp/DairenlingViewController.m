...
@implementation DairenlingViewController
...
-(void)getCheckData{
    ...
    [dataService getPrCheckPlanDataWithAccessToken:AccessToken operationTime:[PowerUtils getOperationTime:@"yyyy-MM-dd-HH:mm:ss"] checkStatusNum:@"1" completeBlock:block loadingView:checkDataTableView showHUD:YES];
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    //delegate响应弹出二级任务列表
    if(_delegate && [_delegate respondsToSelector:@selector(popDairenlingDetailViewController:titleLabel:)]) {
        NSDictionary *checkDataDic = [checkDataList objectAtIndex:indexPath.row];
        [_delegate popDairenlingDetailViewController:[checkDataDic objectForKey:@"PLAN_NUM"] titleLabel:[checkDataDic objectForKey:@"PLAN_NAME"]];
    }
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if(cell == nil) {
        ...
        startTimeLabel.textColor = [UIColor colorWithRed:117/255.0 green:184/255.0 blue:227/255.0 alpha:1];
