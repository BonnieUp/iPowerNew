//
//  ShebeishiwuConditionViewController.m
//  IPowerApp
//
...
@interface ShebeishiwuConditionViewController ()

@end

@implementation ShebeishiwuConditionViewController {
...
- (void)viewDidLoad
{
    ...
    //容器
    containerView = [[UIView alloc]initWithFrame:CGRectMake((ScreenWidth-280)/2, (ScreenHeight-100-cellHei*shebeishiwuList.count-STATUS_BAR_HEIGHT)/2, 280, cellHei*shebeishiwuList.count+100)];
    ...
    //生成存放按钮的view
    UIView *btnContainerView = [[UIView alloc]initWithFrame:CGRectMake(10, 50, containerView.width-20, cellHei*shebeishiwuList.count)];
    ...
    //生成设备大类子按钮
    for(int i=0;i<shebeishiwuList.count;i++) {
        NSDictionary *cellDic = [shebeishiwuList objectAtIndex:i];
        ...
        [cellBtn addTarget:self action:@selector(shebeishiwuBtnOnClick:) forControlEvents:UIControlEventTouchUpInside];
        [containerView addSubview:cellBtn];
        
        [cells addObject:cellBtn];
        //添加分割线
        if(i!= shebeishiwuList.count -1 ) {
...
//设备大类里的字按钮点击事件
-(void)shebeishiwuBtnOnClick:(UIButton *)btn {
    [self resetAllBtn];
    //将选中设置为选中状态
...
-(void)submitHandler {
    if(selectIndex>=0) {
        NSDictionary *selectDic = [shebeishiwuList objectAtIndex:selectIndex];
        if(_delegate && [_delegate respondsToSelector:@selector(submitShebeishiwuCondition:)]) {
            [_delegate submitShebeishiwuCondition:[selectDic objectForKey:@"value"]];
            //关闭窗口
            [self disappearWinow];
        }
    }
...
-(void)setSelectShiwu:(NSString *)selectShiwu{
    if(![_selectShiwu isEqualToString:selectShiwu ]) {
        _selectShiwu = selectShiwu;
        
        //设置当前所选的编号
        NSInteger index;
        for (NSDictionary *item in shebeishiwuList ) {
            if([[item objectForKey:@"value"] isEqualToString:selectShiwu]) {
                index = [shebeishiwuList indexOfObject:item];
                break;
            }
        }
        if(index>=0) {
